...
const ASSET_ENTRY_NAMES = ['vendor', 'codemirror', 'index', 'login', 'database', 'collection', 'document', 'gridfs'];
...
const resolveEntryScript = (entryName, buildFiles) => {
  const exactFileName = buildFiles.find((fileName) => fileName === `${entryName}.js` || fileName === `${entryName}.min.js`);
  if (exactFileName) return exactFileName;

  const hashedFileName = buildFiles.find((fileName) => fileName.startsWith(`${entryName}-`) && fileName.endsWith('.js'));
  if (hashedFileName) return hashedFileName;
...
const getFallbackAssets = () => {
  ...
  if (fs.existsSync(buildPath)) {
    buildFiles = fs.readdirSync(buildPath).filter((fileName) => fileName.endsWith('.js'));
  }

  const hasBuildArtifacts = buildFiles.length > 0;
  return Object.fromEntries(
    ASSET_ENTRY_NAMES.map((entryName) => [entryName, {
      js: hasBuildArtifacts
        ? `public/${resolveEntryScript(entryName, buildFiles)}`
        : `public/fallback/${entryName}.js`,
    }]),
  );
};

const loadBuildAssets = () => {
  const buildAssetsPath = fileURLToPath(new URL('../build-assets.json', import.meta.url));

  try {
    if (fs.existsSync(buildAssetsPath)) {
      return JSON.parse(fs.readFileSync(buildAssetsPath));
    }
  } catch (error) {
    console.warn(`Failed to load assets manifest at ${buildAssetsPath}: ${error.message}`);
  }

  console.warn(`Assets manifest unavailable at ${buildAssetsPath}; falling back to runtime asset discovery.`);
...
const middleware = async function (config) {
  ...
  app.locals.assets = loadBuildAssets();
