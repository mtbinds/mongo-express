...
const DEFAULT_MONGO_CONNECTION_STRING = 'mongodb://localhost:27017/admin';
...
const buildMongoConnection = function (connectionString) {
  if (typeof connectionString === 'string' && connectionString.length > 0) {
    try {
      return new URL(connectionString);
    } catch (error) {
      console.warn(`Invalid MongoDB connection string provided: ${error.message}`);
    }
  }
  return new URL(DEFAULT_MONGO_CONNECTION_STRING);
};
...
const router = async function (config) {
  ...
  appRouter.use('/public/css', express.static(fileURLToPath(new URL('../public/stylesheets', import.meta.url))));
  appRouter.use('/public/img', express.static(fileURLToPath(new URL('../public/images', import.meta.url))));
  appRouter.use('/public', express.static(fileURLToPath(new URL('../public', import.meta.url))));
  ...
  const sessionSecret = config.site.sessionSecret || config.site.cookieSecret || 'mongo-express-default-session-secret';
  if (!config.site.sessionSecret) {
    console.warn('ME_CONFIG_SITE_SESSIONSECRET is not set; using fallback session secret.');
  }
  ...
  appRouter.all('/{*splat}', async function (req, res, next) {
    if (mongo === null) {
      const connection = buildMongoConnection(config.mongodb.connectionString);
      if (req.method === 'POST') {
        const {
