...
const redactConnectionString = function (connectionString) {
  if (typeof connectionString !== 'string' || connectionString.length === 0) {
    return '<missing connection string>';
  }
  return connectionString.replace(/(mongo.*?:\/\/.*?:).*?@/, '$1****@');
};
...
const connect = async function (config) {
  ...
  connectionData.clients = await Promise.all(connections.map(async (connectionInfo, index) => {
    ...
    try {
      ...
      console.error(`Could not connect to database using connectionString: ${redactConnectionString(connectionString)}`);
      throw error;
    }
  }));
